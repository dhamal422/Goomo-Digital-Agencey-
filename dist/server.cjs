...
userSchema.methods.comparePassword = async function(candidatePassword) {
  try {
    return await import_bcryptjs.default.compare(candidatePassword, this.password);
  } catch (error) {
    return false;
  }
};
...
var servicesStore = [...INITIAL_SERVICES];
var settingsStore = {
  servicesWhatsAppNumber: "+919876543210"
};
var testimonialsStore = [
  {
    id: "testi-1",
...
var leadsStore = [
  ...
  },
  {
    ...
    timestamp: new Date(Date.now() - 36e5 * 36).toISOString(),
    status: "Closed"
  }
];
async function startServer() {
  ...
  app.get("/api/services", (req, res) => {
    ...
    if (search && typeof search === "string") {
      ...
      result = result.filter(
        (s) => s.title.toLowerCase().includes(q) || s.shortDesc.toLowerCase().includes(q) || s.category.toLowerCase().includes(q)
      );
    }
  ...
  app.put("/api/services/:slug", (req, res) => {
    const { slug } = req.params;
    ...
    const updatedService = {
      ...servicesStore[index],
      title: title !== void 0 ? title : servicesStore[index].title,
      shortDesc: shortDesc !== void 0 ? shortDesc : servicesStore[index].shortDesc,
      fullDeepContent: fullDeepContent !== void 0 ? fullDeepContent : servicesStore[index].fullDeepContent,
      basePrice: basePrice !== void 0 ? Number(basePrice) : servicesStore[index].basePrice,
      category: category !== void 0 ? category : servicesStore[index].category,
    ...
    try {
      ...
      if (isMongoConnected()) {
        ...
        const newUserDoc = new UserModel({
          ...
          email: cleanEmail,
          password,
          // Mongoose pre-save hook will bcrypt hash this
          role: "user",
          viewRatesUnlocked: true
        });
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      if (isMongoConnected()) {
        ...
        const isMatch2 = await userDoc.comparePassword(password);
        if (!isMatch2) {
          return res.status(401).json({ success: false, error: "Invalid email or password." });
        }
      ...
      if (user.password.startsWith("$2a$") || user.password.startsWith("$2b$")) {
        isMatch = await import_bcryptjs2.default.compare(password, user.password);
      } else {
        isMatch = user.password === password;
        if (isMatch) {
          user.password = await import_bcryptjs2.default.hash(password, 10);
        }
      }
      if (!isMatch) {
        return res.status(401).json({ success: false, error: "Invalid email or password." });
  ...
  app.post("/api/admin/clients", async (req, res) => {
    try {
      ...
      if (isMongoConnected()) {
        const existing2 = await UserModel.findByEmail(cleanEmail);
        if (existing2) {
  ...
  app.post("/api/admin/change-password", async (req, res) => {
    try {
      ...
      if (currentPassword && adminUser.password) {
        const isMatch = adminUser.password.startsWith("$2") ? await import_bcryptjs2.default.compare(currentPassword, adminUser.password) : adminUser.password === currentPassword;
        if (!isMatch) {
          return res.status(400).json({ success: false, error: "Incorrect current password." });
        }
      }
      adminUser.password = await import_bcryptjs2.default.hash(newPassword, 10);
  ...
  let paymentsStore = [];
  app.post("/api/payment/razorpay/capture", async (req, res) => {
    try {
      ...
      if (capturedStatus) {
        ...
        const paymentRecord = {
          ...
          payment_create_date: (/* @__PURE__ */ new Date()).toISOString(),
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`\u{1F680} Goomo Digital Agency Server running on http://0.0.0.0:${PORT}`);
  });
}
startServer();
